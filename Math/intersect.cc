...
int  BBoxPlaneIntersect (const BBox *theBBox, Plane *thePlane) {
	...
	int posCercano = thePlane->whichSide(cercano);
	int posLejano = thePlane->whichSide(lejano);

	if (posCercano==posLejano) {
		if (posCercano >0) return +IREJECT;
		if (posCercano <0) return -IREJECT;
	}
