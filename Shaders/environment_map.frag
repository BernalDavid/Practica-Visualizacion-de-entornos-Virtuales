...
void main() {
	...
	//cambios para Environment mapping
	I = normalize(campos - f_positionw);
	R = 2 * dot(f_normalw, I)*f_normalw - I;	// R= 2*(nw*I)*nw-i
	//cambiar la z de signo para pasarlo a sistema levogiro
	R = vec3(R.xy, -R.z);
	vec4 texel = textureCube(envmap, R);


	gl_FragColor = vColor * texColor *texel;
}
