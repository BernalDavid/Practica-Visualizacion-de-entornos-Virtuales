...
void aporte_direccional(in int i, in vec3 l, in vec3 n, in vec3 v, inout vec3 acumulador_difuso, inout vec3 acumulador_especular, in vec4 texel) {
	...
	if (NoL > 0.0) {
		...
		//factor_especular = dot(n,l)* pow(factor_especular,m)  * texel.xyz * theLights[i].specular;
		acumulador_especular += NoL * especular_factor(n, l, v, theMaterial.shininess) * texel.rgb * theLights[i].specular;
	}
}

void aporte_posicional(in int i, in vec3 l, in vec3 n, in vec3 v, inout vec3 acumulador_difuso, inout vec3 acumulador_especular, in vec4 texel) {
	...
	if (NoL > 0.0) {
		...
		acumulador_especular += NoL * theLights[i].specular * especular_factor(n,l,v, theMaterial.shininess) * texel.rgb * fdist;

	}
}

void aporte_spotlight(in int i, in vec3 l, in vec3 n, in vec3 v, inout vec3 acumulador_difuso, inout vec3 acumulador_especular, in vec4 texel) {
	//LAMBERT
	float NoL = lambert_factor(n,l);
	...
	if (NoL > 0.0) {
		...
		if (coseno > theLights[i].cosCutOff) {
			...
			acumulador_especular += NoL * theLights[i].specular * especular_factor(n, l, v, theMaterial.shininess) *texel.rgb * cspot;
		}
	}	
...
void main() {
	...
	//sustituir theMaterial.specular por texel
	...
	for(int i=0; i < active_lights_n; ++i) {
		...
		if (theLights[i].position.w == 0.0) {
			...
			aporte_direccional(i, L, N, V, acumulador_difuso, acumulador_especular, texel);
		}
		//CASO DE LA LUZ POSICIONAL
			...
			aporte_posicional(i, L, N, V, acumulador_difuso, acumulador_especular, texel);
		}
		//CASO DE LA SPOT
		else {
			...
			aporte_spotlight(i, L, N, V, acumulador_difuso, acumulador_especular, texel);
		}
	}
